...
展示入口通过 iframe 横向轮播聚合 5 个移动端竖屏页面，模拟用户连续刷卡浏览与即时使用的完整路径。入口支持居中翻页按钮、鼠标滚轮、键盘方向键与拖拽翻页，评审可直接打开入口文件体验完整作品。
...
| 未来 AI 输入信号 | 当前模拟状态 / 模型输出 | 即时 UI 动作 |
| --- | --- | --- |
...
`index.html` 使用 iframe 聚合 5 个业务页面，仅在页面切换到对应索引时写入 `src`，减少首屏加载压力。容器支持居中上一页/下一页按钮、鼠标滚轮、键盘方向键和 pointer 拖拽，模拟短视频 feed 中连续浏览的操作方式；按钮点击会阻止事件继续冒泡，避免翻页按钮被误判为拖拽起点。
...
滚轮翻页额外做了阈值、冷却时间和 delta 归一化，避免触控板小幅滚动或连续滚动导致误触发；同源 iframe 加载完成后也会绑定滚轮事件，让用户把鼠标停在页面内容上时仍能连续切换 feed。iframe 内部绑定使用 `WeakSet` 防重复，跨域场景通过 `try/catch` 安全跳过，滚轮监听使用 `{ passive: false }` 保证必要时可以阻止默认滚动。
...
const normalizeWheelDelta = (event) => {
  ...
  if (event.deltaMode === 2) return dominantDelta * window.innerHeight;
  return dominantDelta;
};

const handleWheel = (event) => {
  if (event.ctrlKey || isDragging) return;
